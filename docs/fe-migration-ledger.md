...
| security      | `com.socialapp.security`      | 17  | `/profile/professional` (GET+PUT) **không** thuộc security — `ProfessionalProfileController` nằm trong package `knowledge`. Lọc path `/v1/api/profile*` sẽ ra 19, đúng là 17. | in progress (8/17) | in progress (8/17) | in progress (8/17) | in progress (login+register+oauth) | not started | not started    | not started |
