...
Domain đã có file findings: `posts` · `newsfeed` · `security` · `friendships` · `reputation` · `notifications` · `chat` · `shared`
...
| chat          | `com.socialapp.chat`          | 1   | BE cấp token **Stream Chat**; FE Twilio cũ chết ở P2.7d. BE đã sửa xong R1–R5 (2026-07-27). **R8 mới**: Stream đòi user kia tồn tại trước khi tạo channel → chưa nhắn được người chưa từng mở chat. Vượt trần UI → tách **P2.7a / c-1 / c-2 / d** |                  | done (1/1)       | done (1/1)       | done (1/1)                                                 | not started             | not started                                                     | not started | not started |
