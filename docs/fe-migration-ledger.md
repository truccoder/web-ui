...
| ------------- | ----------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | ---------------- | ------------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------- | --------------------------------------------------------------- | --------------- |
| posts         | `com.socialapp.posts`         | 22  | `EventController` (`/events`) nằm trong package `posts` — mirror theo package, không tách `features/events/`. `acceptAnswer` nằm ở `PostController` chứ không phải `CommentController` → thuộc chu kỳ 1 | **done (22/22)** | **done (22/22)** | in progress (14/22) | in progress (composer 7/8 loại; chu kỳ 2 UI done: card + 7/7 body + reaction + comment + quiz/sửa/xoá/accept) | chu kỳ 1+2 done (`/newsfeed`) | chu kỳ 1+2 done (EVENT + bookstore giữ cầu tạm → P2.4″d/P2.10)  | chu kỳ 1+2 done |
...
| `/newsfeed`            | `app/(main)/newsfeed/page.tsx`            | **newsfeed** | posts, bookstore, security | **span 4 domain** — P3.1. Composer (P2.4d) + card/body/reaction/comment (P2.4′d) đã sang `features/posts`; còn legacy: **query feed** (`useNewsfeed`, P2.5), adapter `feed-post.tsx` (→ P2.5), cầu tạm EVENT (P2.4″d) và bookstore (P2.10) |
