...
| ------------- | ----------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ------------------- | ------------------ | -------------------------------------- | --------------------------- | --------------------------------------------------------------- | ------------- |
| posts         | `com.socialapp.posts`         | 22  | `EventController` (`/events`) nằm trong package `posts` — mirror theo package, không tách `features/events/`. `acceptAnswer` nằm ở `PostController` chứ không phải `CommentController` → thuộc chu kỳ 1 | in progress (14/22) | in progress (14/22) | in progress (7/22) | in progress (composer 7/8 loại + quiz) | chu kỳ 1 done (`/newsfeed`) | chu kỳ 1 done (composer; EVENT giữ cầu tạm → P2.4″d)            | chu kỳ 1 done |
