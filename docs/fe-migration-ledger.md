...
| `/newsfeed`            | `app/(main)/newsfeed/page.tsx`            | **newsfeed** | posts, bookstore, security | **span 4 domain** — P3.1. **Toàn bộ phần posts đã sang `features/posts`** (composer 8/8 loại, mặt đọc, RSVP/lịch). Còn legacy: **query feed** (`useNewsfeed`, P2.5), adapter `feed-post.tsx` (→ P2.5), cầu tạm bookstore `book-post-actions.tsx` (→ P2.10) |
