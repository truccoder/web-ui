...
| moderation     | 4/4             | 4/4             | **ĐÃ XOÁ HẾT ở P2.15cd**: `components/moderation/` (5 file), `lib/api/moderation.ts`, `lib/hooks/use-moderation.ts`, 8 type ở `lib/types/index.ts`, và khoá i18n `admin.moderation.*` mồ côi. `src/components/` giờ chỉ còn `ui/` (shadcn) + `service-worker-register.tsx` |
...
| roadmap        | 8/8             | —               | **n/a — chưa từng có code legacy.** Domain chưa bao giờ có UI, nên `d` tạo route mới (`/roadmap`, `/admin/roadmap`) thay vì rewire; không có gì để xoá. Cùng tình huống `notifications` (P2.6cd) và `knowledge` (P2.11d)                                                   |
| github         | 5/5             | —               | **n/a — chưa từng có code legacy.** `cd` tạo route mới `/github` thay vì rewire; không có gì để xoá                                                                                                                                                                        |
| matchmaking    | 5/5             | —               | **n/a — chưa từng có code legacy.** UI hoãn vì B24, nên chưa có route nào                                                                                                                                                                                                  |
