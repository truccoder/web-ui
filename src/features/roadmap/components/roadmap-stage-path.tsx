...
export function RoadmapStagePath({
  ...
  return (
    <div className={cn('flex flex-col gap-[var(--nx-space-block)]', className)}>
      {/**
       * `items-start` IS LOAD-BEARING, not a default worth changing: stages hold different numbers
       * of skills, so stretching them would make every card as tall as the tallest and leave the
       * short ones half empty. Aligned at the top, the chain reads as a row of stages that happen
       * to differ in depth — which is the truth about the data.
       *
            ...
            <Fragment key={stage.id}>
              ...
              <li className="min-w-0 flex-1 [flex-basis:var(--nx-path-stage)] [max-width:var(--nx-path-stage-max)]">
                <Card className="flex h-full flex-col gap-3">
                  {/* 32 is the row unit — a minimum rather than a height, so a wrapped Vietnamese
                      stage name grows the row instead of clipping. */}
                  <div className="flex min-h-8 items-center gap-2">
                    <h3 className="min-w-0 flex-1 truncate text-nx-ui font-semibold text-nx-text-primary">
                      {stage.name}
                    </h3>
                    {skills.length > 0 && (
