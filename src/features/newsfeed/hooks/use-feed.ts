 ...
 */
export function usePost(postId: number | undefined, options?: { watchModeration?: boolean }) {
  return useQuery({
    ...
    retry: false,
    /**
     * A post the author has just written is re-read until moderation settles.
     *
     * `FeedPostDataDto.moderationStatus` is the answer itself, so this polls the post rather
     * than guessing from somewhere else: `PENDING_MODERATION` is the transient state the AI
     * check clears within a second or two, and anything else — `APPROVED`, `PENDING_REVIEW`,
     * `REJECTED` — is a resting state that no amount of asking again will change.
     *
     * Capped at {@link MODERATION_MAX_POLLS} so a page left open does not poll for ever.
     */
    refetchInterval: (query) => {
      if (!options?.watchModeration) return false;
      if (query.state.data?.moderationStatus !== 'PENDING_MODERATION') return false;
      return query.state.dataUpdateCount >= MODERATION_MAX_POLLS ? false : MODERATION_POLL_MS;
    },
    refetchOnWindowFocus: false,
  });
}

/** Milliseconds between moderation re-reads — the AI check settles 1–2s after `AFTER_COMMIT`. */
const MODERATION_POLL_MS = 2000;
/** How many re-reads before the page stops asking (~40s) and lets the notice stand. */
const MODERATION_MAX_POLLS = 20;
