 ...
 *
 * `language` is a free-form `String` on the backend (no enum). The option list now lives in
 * `lib/code-languages` because the READER side keys its grammar registry off the same slugs —
 * `CodeSnippetBody` colours a snippet only when its language is one of these. Two copies would
 * drift silently: a slug added here and not there is a post that stores fine and never colours.
 * Nothing breaks if the backend receives a value that is not in the list; it renders plain.
 */
...
export function CodeSnippetFields({ value, onChange }: CodeSnippetFieldsProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <Select
        size="sm"
        // `wrapperClassName`, NOT `className` — see the note on `Select`'s chevron. Sizing the
        // field itself left the arrow outside the box.
