...
// `text-nx-*` carries the token's own line-height, which would beat a separate
// `leading-none`; the `/none` modifier sets both at once, as the specimen does.
const sizeStyles: Record<NonNullable<RepScoreProps['size']>, string> = {
  sm: 'h-5 gap-[5px] px-1.5 text-nx-micro/none',
  md: 'h-6 gap-[7px] px-2 text-nx-caption/none',
  lg: 'h-8 gap-[9px] px-2.5 text-nx-body/none',
};
 ...
 */
export function formatRep(score = 0): string {
