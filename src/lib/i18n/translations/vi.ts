import type { Messages } from './en';

export const vi: Messages = {
  app: {
    name: 'Elite Nexus',
  },

  payment: {
    invalidTitle: 'Liên kết thanh toán không hợp lệ',
    invalidDesc: 'Liên kết này thiếu thông tin đơn hàng.',
    checkingTitle: 'Đang xác nhận thanh toán...',
    checkingDesc: 'Vui lòng đợi trong khi chúng tôi xác minh giao dịch với MoMo.',
    failedTitle: 'Thanh toán thất bại',
    failedDesc: 'Không thể hoàn tất thanh toán của bạn. Vui lòng thử lại.',
    successTitle: 'Thanh toán thành công',
    successDesc: 'Cảm ơn bạn! Bạn đã có toàn quyền truy cập cuốn sách này.',
    backToNewsfeed: 'Về trang bảng tin',
  },

  admin: {
    title: 'Quản trị kiểm duyệt',
    moderation: {
      title: 'Kiểm duyệt',
      subtitle: 'Xem lại bài viết, lịch sử kiểm duyệt và quản lý người dùng bị cấm',
      tabs: {
        posts: 'Bài viết',
        logs: 'Nhật ký',
        banned: 'Người dùng bị cấm',
      },
      filterPostId: 'Post ID',
      filterUserId: 'User ID',
      filterStatus: 'Trạng thái',
      allStatuses: 'Tất cả trạng thái',
      pageOf: 'Trang ${page} / ${totalPages}',
      retry: 'Thử lại',
      error: 'Không thể tải dữ liệu. Vui lòng thử lại.',
      empty: {
        title: 'Không tìm thấy bài viết nào',
        desc: 'Thử điều chỉnh bộ lọc ở trên.',
      },
      noLogs: 'Không có nhật ký kiểm duyệt nào',
      noBannedUsers: 'Không có người dùng bị cấm nào',
      viewHistory: 'Lịch sử (${count})',
      noHistory: 'Chưa có lịch sử kiểm duyệt',
      toxicity: 'Độc hại',
      imageSafeScore: 'Điểm không an toàn của ảnh',
      feedbackPlaceholder: 'Phản hồi cho tác giả (không bắt buộc)...',
      defaultFeedback: 'Gemini free tier out of quota',
      approve: 'Duyệt',
      reject: 'Từ chối',
      banned: 'Đang bị cấm · còn ${remaining}',
      notBanned: 'Không bị cấm',
      banCount: 'Số lần bị cấm: ${count}',
      triggeringPosts: 'Bài viết gây cấm',
    },
  },

  nav: {
    newsfeed: 'Bảng tin',
    trending: 'Xu hướng',
    friends: 'Bạn bè',
    friendsAll: 'Tất cả bạn bè',
    friendsSuggestions: 'Gợi ý',
    friendsRequests: 'Lời mời kết bạn',
    friendsBirthdays: 'Sinh nhật',
    chats: 'Chats',
    knowledge: 'Kiến thức',
    profile: 'Trang cá nhân',
    dashboard: 'Dashboard',
    logout: 'Đăng xuất',
  },

  auth: {
    email: 'Email',
    emailPlaceholder: 'ten@example.com',
    password: 'Mật khẩu',
    fullname: 'Họ và tên',
    fullNamePlaceholder: 'Nguyễn Văn A',
    login: {
      title: 'Chào mừng trở lại',
      subtitle: 'Đăng nhập vào tài khoản Elite Nexus của bạn',
      forgotPassword: 'Quên mật khẩu?',
      passwordPlaceholder: 'Nhập mật khẩu của bạn',
      submit: 'Đăng nhập',
      submitting: 'Đang đăng nhập...',
      noAccount: 'Chưa có tài khoản?',
      signUp: 'Đăng ký',
    },
    register: {
      title: 'Tạo tài khoản',
      subtitle: 'Tham gia Elite Nexus và bắt đầu kết nối với mọi người',
      profilePicture: 'Ảnh đại diện',
      optional: '(tùy chọn)',
      changePhoto: 'Đổi ảnh',
      uploadPhoto: 'Tải ảnh lên',
      photoFormats: 'JPG, PNG hoặc WebP. Tối đa 5MB.',
      passwordPlaceholder: 'Ít nhất 8 ký tự',
      submit: 'Tạo tài khoản',
      submitting: 'Đang tạo...',
      alreadyHaveAccount: 'Đã có tài khoản?',
      signIn: 'Đăng nhập',
    },
    forgotPassword: {
      title: 'Quên mật khẩu?',
      subtitle: 'Nhập email của bạn và chúng tôi sẽ gửi link đặt lại mật khẩu',
      submit: 'Gửi link đặt lại',
      submitting: 'Đang gửi...',
      successTitle: 'Kiểm tra email của bạn',
      successDesc:
        'Chúng tôi đã gửi link đặt lại mật khẩu đến địa chỉ email của bạn. Vui lòng kiểm tra hộp thư.',
      backToLogin: 'Quay lại đăng nhập',
    },
  },

  newsfeed: {
    title: 'Bảng tin',
    subtitle: 'Cập nhật mới nhất từ bạn bè và những người bạn theo dõi',
    error: 'Không thể tải bài viết. Vui lòng thử lại.',
    retry: 'Thử lại',
    empty: {
      title: 'Chưa có bài viết nào',
      desc: 'Hãy là người đầu tiên đăng bài hoặc kết bạn thêm để xem tin tức mới!',
    },
    allLoaded: 'Bạn đã xem hết bài viết rồi 🎉',
  },

  search: {
    placeholder: 'Tìm kiếm mọi người, bài viết và sách...',
    error: 'Tìm kiếm thất bại. Vui lòng thử lại.',
    empty: 'Không có kết quả cho "${query}"',
    usersSection: 'Mọi người (${count})',
    postsSection: 'Bài viết (${count})',
    title: 'Tìm kiếm',
    resultsFor: 'Kết quả cho "${query}"',
    prompt: 'Nhập nội dung và nhấn Enter để tìm kiếm',
  },

  trending: {
    title: 'Xu hướng',
    subtitle: 'Những bài viết công nghệ nổi bật trên khắp mạng',
    error: 'Không thể tải nội dung xu hướng. Vui lòng thử lại.',
    retry: 'Thử lại',
    empty: {
      title: 'Chưa có nội dung xu hướng',
      desc: 'Quay lại sau để xem những bài viết mới nhất.',
    },
    allLoaded: 'Bạn đã xem hết nội dung xu hướng rồi 🎉',
    allCategories: 'Tất cả',
    timeRange: {
      today: 'Hôm nay',
      week: 'Tuần này',
      month: 'Tháng này',
    },
    categories: {
      OPENSOURCE: 'Mã nguồn mở',
      EVENT: 'Sự kiện',
      NEW_TECH: 'Công nghệ mới',
      REGULATION: 'Quy định',
      MINDSET: 'Tư duy',
      TOOL: 'Công cụ',
      CAREER: 'Sự nghiệp',
      OTHER: 'Khác',
    },
    sources: {
      HACKER_NEWS: 'Hacker News',
      DEV_TO: 'DEV Community',
      GITHUB: 'GitHub',
      REDDIT: 'Reddit',
      MEDIUM: 'Medium',
      HBR: 'HBR',
    },
  },

  createPost: {
    placeholder: '${fullname} đang nghĩ gì vậy?',
    photo: 'Ảnh',
    posting: 'Đang đăng...',
    post: 'Đăng',
    visibility: {
      PUBLIC: 'Công khai',
      FRIENDS: 'Bạn bè',
      PRIVATE: 'Chỉ mình tôi',
    },
    event: {
      button: 'Sự kiện',
      title: 'Tên sự kiện',
      description: 'Mô tả sự kiện',
      startTime: 'Thời gian bắt đầu',
      endTime: 'Thời gian kết thúc',
      endTimeAuto: 'Sẽ tự động tính',
      endMode: {
        DEFAULT: '+1 giờ',
        PLUS_2H: '+2 giờ',
        PLUS_3H: '+3 giờ',
        CUSTOM: 'Tuỳ chỉnh',
      },
      location: 'Địa điểm',
      locationError: 'Không thể xác định địa điểm này',
      onlineUrl: 'Link trực tuyến (tuỳ chọn)',
      maxAttendees: 'Số người tham gia tối đa (tuỳ chọn)',
      cancel: 'Huỷ sự kiện',
    },
    book: {
      button: 'Sách',
      title: 'Tên sách',
      description: 'Mô tả sách',
      file: 'Tệp sách (PDF hoặc EPUB)',
      cover: 'Ảnh bìa (tuỳ chọn)',
      price: 'Giá (VND, để trống nếu miễn phí)',
      previewPages: 'Số trang xem trước (bắt buộc với sách có phí)',
      cancel: 'Huỷ sách',
      fileRequired: 'Vui lòng chọn tệp PDF hoặc EPUB',
      fileInvalidFormat: 'Chỉ hỗ trợ định dạng PDF và EPUB',
      chooseFile: 'Chọn tệp',
      chooseCover: 'Chọn ảnh bìa',
      removeFile: 'Xoá',
      previewError: 'Không thể xem trước tệp PDF này',
      previewPageCount: '${count} trang',
    },
  },

  post: {
    like: 'Thích',
    comment: 'Bình luận',
    share: 'Chia sẻ',
    justNow: 'Vừa xong',
    minutesAgo: '${minutes} phút trước',
    hoursAgo: '${hours} giờ trước',
    daysAgo: '${days} ngày trước',
    commentPlaceholder: 'Viết bình luận...',
    olderCommentsHidden: 'Chưa thể xem các bình luận trước đó.',
    send: 'Gửi',
    commentsError: 'Không thể tải bình luận',
    noComments: 'Chưa có bình luận nào. Hãy là người đầu tiên!',
    deleteComment: 'Xoá bình luận',
    editComment: 'Sửa bình luận',
    saveComment: 'Lưu',
    cancelEditComment: 'Huỷ',
    menu: {
      edit: 'Sửa bài viết',
      delete: 'Xoá bài viết',
      confirmDeleteTitle: 'Xoá bài viết này?',
      confirmDeleteDesc: 'Hành động này không thể hoàn tác.',
      editTitle: 'Sửa bài viết',
      save: 'Lưu',
      saving: 'Đang lưu...',
      cancel: 'Huỷ',
      confirm: 'Xoá',
    },
    event: {
      status: {
        upcoming: 'Sắp diễn ra',
        ongoing: 'Đang diễn ra',
        past: 'Đã kết thúc',
      },
      joinOnline: 'Tham gia trực tuyến',
      maxAttendees: 'Tối đa ${count} người tham gia',
      rsvp: {
        going: 'Tham gia',
        interested: 'Quan tâm',
        notGoing: 'Không tham gia',
      },
      attendeeCount: '${count} người tham gia',
      addToCalendar: 'Thêm vào Google Calendar',
      connectCalendar: 'Kết nối Google Calendar',
      exportIcs: 'Tải file .ics',
      icsError: 'Không thể tải file lịch',
      attendeesTitle: 'Người tham dự',
      attendeesEmpty: 'Chưa có phản hồi nào',
      attendeesError: 'Không thể tải danh sách người tham dự',
      unknownUser: 'Người dùng #${id}',
    },
    book: {
      free: 'Miễn phí',
      reviewCount: '${count} đánh giá',
      preview: 'Xem trước',
      hidePreview: 'Ẩn xem trước',
      previewUrlError: 'Không thể tải bản xem trước',
      openPreview: 'Mở bản xem trước',
      reviews: 'Đánh giá',
      hideReviews: 'Ẩn đánh giá',
      reviewsError: 'Không thể tải đánh giá',
      noReviews: 'Chưa có đánh giá nào',
      feedbackPlaceholder: 'Viết đánh giá (không bắt buộc)',
      submitReview: 'Gửi',
      readerLoadError: 'Không thể mở sách để đọc',
      previousPage: 'Trước',
      nextPage: 'Sau',
      pageIndicator: 'Trang ${current} / ${total}',
      buy: 'Mua',
      download: 'Tải xuống',
    },
  },

  friends: {
    title: 'Bạn bè',
    all: {
      title: 'Tất cả bạn bè',
      subtitle: '${count} bạn bè',
      empty: {
        title: 'Chưa có bạn bè',
        desc: 'Những người bạn kết nối sẽ xuất hiện ở đây.',
      },
      allLoaded: 'Bạn đã xem hết danh sách bạn bè',
    },
    suggestions: {
      title: 'Gợi ý kết bạn',
      subtitle: 'Những người bạn có thể biết',
      addFriend: 'Thêm bạn bè',
      ignore: 'Bỏ qua',
      requestSent: 'Đã gửi lời mời',
      suggestedForYou: 'Gợi ý cho bạn',
      mutualFriends: '${count} bạn chung',
      empty: {
        title: 'Không có gợi ý nào',
        desc: 'Chúng tôi sẽ gợi ý thêm bạn bè khi có người phù hợp với bạn.',
      },
    },
    requests: {
      title: 'Lời mời kết bạn',
      subtitle: 'Quản lý lời mời kết bạn của bạn',
      tabReceived: 'Đã nhận',
      tabSent: 'Đã gửi',
      confirm: 'Xác nhận',
      delete: 'Xóa',
      cancelRequest: 'Hủy lời mời',
      awaiting: 'Đang chờ phản hồi',
      receivedEmpty: {
        title: 'Không có lời mời nào',
        desc: 'Khi ai đó gửi lời mời kết bạn cho bạn, chúng sẽ xuất hiện ở đây.',
      },
      sentEmpty: {
        title: 'Chưa gửi lời mời nào',
        desc: 'Các lời mời kết bạn bạn đã gửi sẽ xuất hiện ở đây.',
      },
    },
    birthdays: {
      title: 'Sinh nhật',
      subtitle: 'Đừng bỏ lỡ sinh nhật của bạn bè',
      today: 'Sinh nhật hôm nay',
      todayMsg: 'Hôm nay là sinh nhật của họ!',
      thisWeek: 'Sinh nhật tuần này',
      upcoming: 'Sinh nhật sắp tới',
      wishHappy: 'Chúc mừng',
      tomorrow: 'Ngày mai',
      daysAway: '${days} ngày nữa',
      noFriends: {
        title: 'Chưa có bạn bè nào',
        desc: 'Thêm bạn bè để xem sinh nhật của họ ở đây.',
      },
    },
  },

  dashboard: {
    welcome: 'Chào mừng trở lại',
    welcomeName: 'Chào mừng trở lại, ${fullname}',
    subtitle: 'Đây là những gì đang xảy ra trong mạng lưới của bạn',
    stats: {
      friends: 'Bạn bè',
      friendsDesc: 'Tổng kết nối',
      sent: 'Đã gửi',
      sentDesc: 'Lời mời chờ phản hồi',
      unread: 'Thông báo',
      unreadDesc: 'Thông báo chưa đọc',
      pending: 'Đang chờ',
      pendingDesc: 'Lời mời kết bạn',
    },
    requests: {
      title: 'Lời mời kết bạn',
      desc: 'Những người muốn kết nối với bạn',
      empty: 'Không có lời mời nào',
      accept: 'Chấp nhận',
      reject: 'Từ chối',
    },
    suggestions: {
      title: 'Người bạn có thể biết',
      desc: 'Mở rộng mạng lưới của bạn',
      empty: 'Hiện không có gợi ý nào',
      add: 'Thêm',
    },
  },

  profile: {
    title: 'Trang cá nhân',
    subtitle: 'Quản lý cài đặt tài khoản của bạn',
    uploadHint: 'Nhấp để tải ảnh mới lên (JPG, PNG, WebP, tối đa 5MB)',
    id: 'ID: ${id}',
    tabs: {
      info: 'Thông tin cá nhân',
      password: 'Đổi mật khẩu',
      notifications: 'Thông báo',
    },
    info: {
      title: 'Thông tin cá nhân',
      desc: 'Cập nhật tên hiển thị của bạn',
      fullname: 'Họ và tên',
      save: 'Lưu thay đổi',
      saving: 'Đang lưu...',
    },
    password: {
      title: 'Đổi mật khẩu',
      desc: 'Chọn mật khẩu mạnh để bảo vệ tài khoản của bạn',
      currentPassword: 'Mật khẩu hiện tại',
      currentPlaceholder: 'Nhập mật khẩu hiện tại',
      newPassword: 'Mật khẩu mới',
      newPlaceholder: 'Ít nhất 8 ký tự',
      confirm: 'Xác nhận mật khẩu',
      confirmPlaceholder: 'Nhập lại mật khẩu',
      update: 'Cập nhật mật khẩu',
      updating: 'Đang cập nhật...',
    },
  },

  notifications: {
    title: 'Thông báo',
    markAllRead: 'Đánh dấu tất cả đã đọc',
    empty: 'Chưa có thông báo nào',
    loadMore: 'Tải thêm',
    error: 'Không thể tải thông báo',
    prefs: {
      title: 'Cài đặt thông báo',
      desc: 'Chọn cách và thời điểm bạn muốn nhận thông báo.',
      push: 'Thông báo đẩy',
      pushDesc: 'Nhận thông báo trên trình duyệt',
      email: 'Thông báo qua email',
      emailDesc: 'Nhận thông báo qua email',
      emailFrequency: 'Tần suất email',
      frequency: {
        INSTANT: 'Ngay lập tức',
        DAILY_DIGEST: 'Tổng hợp hằng ngày',
        WEEKLY_DIGEST: 'Tổng hợp hằng tuần',
        NONE: 'Không bao giờ',
      },
      mutedTypes: 'Loại thông báo đã tắt',
      mutedTypesDesc: 'Bạn sẽ không nhận thông báo cho các loại được đánh dấu.',
      save: 'Lưu cài đặt',
      saving: 'Đang lưu...',
      error: 'Không thể tải cài đặt thông báo',
    },
    types: {
      POST_LIKED: 'Cảm xúc bài viết',
      POST_COMMENTED: 'Bình luận bài viết',
      POST_SHARED: 'Chia sẻ bài viết',
      POST_TAGGED: 'Gắn thẻ trong bài viết',
      FRIEND_REQUEST: 'Lời mời kết bạn',
      FRIEND_ACCEPTED: 'Chấp nhận kết bạn',
      EVENT_RSVP: 'RSVP sự kiện',
      EVENT_REMINDER: 'Nhắc nhở sự kiện',
      BOOK_REVIEW: 'Đánh giá sách',
      BOOK_PURCHASED: 'Mua sách',
      SYSTEM: 'Thông báo hệ thống',
    },
  },

  knowledge: {
    title: 'Kiến thức',
    subtitle: 'Giải thích AI, hồ sơ chuyên môn và access token',
    tabs: {
      library: 'Thư viện',
      profile: 'Hồ sơ chuyên môn',
      tokens: 'Access token',
    },
    library: {
      count: '${count} giải thích đã lưu',
      empty: {
        title: 'Chưa có giải thích nào được lưu',
        desc: 'Dùng nút "Giải thích AI" trên bài viết ở bảng tin để lưu giải thích vào đây.',
      },
      concepts: 'Khái niệm',
      prerequisites: 'Kiến thức cần có',
      complexity: 'Độ phức tạp ${score}/10',
      viewOriginal: 'Nội dung bài viết gốc',
      links: 'Đọc thêm',
      showMore: 'Xem thêm',
      showLess: 'Thu gọn',
      error: 'Không thể tải thư viện của bạn',
    },
    profile: {
      desc: 'Dùng để cá nhân hoá giải thích AI và cải thiện gợi ý kết bạn.',
      jobTitle: 'Chức danh',
      jobTitlePlaceholder: 'VD: Backend Engineer',
      seniority: 'Cấp bậc',
      yearsOfExperience: 'Số năm kinh nghiệm',
      primaryRole: 'Vai trò chính',
      explanationStyle: 'Phong cách giải thích',
      techStack: 'Tech stack đã biết',
      techStackPlaceholder: 'java, spring, react (phân cách bằng dấu phẩy)',
      interestedDomains: 'Lĩnh vực quan tâm',
      interestedDomainsPlaceholder: 'fintech, e-commerce (phân cách bằng dấu phẩy)',
      save: 'Lưu hồ sơ',
      saving: 'Đang lưu...',
      error: 'Không thể tải hồ sơ chuyên môn',
    },
    tokens: {
      desc: 'Personal access token cho phép công cụ ngoài (VD: đồng bộ Obsidian vault) đọc và ghi thư viện kiến thức của bạn.',
      create: 'Tạo token',
      createTitle: 'Tạo access token',
      name: 'Tên token',
      namePlaceholder: 'VD: Obsidian vault',
      expiresInDays: 'Hết hạn sau (ngày)',
      expiresHint: 'Để trống nếu không hết hạn',
      permission: 'Quyền với vault',
      permissionWriteOnly: 'Chỉ ghi',
      permissionBidirectional: 'Hai chiều',
      created: 'Sao chép token ngay — nó sẽ không hiển thị lại lần nào nữa.',
      copy: 'Sao chép',
      copied: 'Đã sao chép',
      revoke: 'Thu hồi',
      revoked: 'Đã thu hồi token',
      empty: 'Chưa có token nào',
      lastUsed: 'Dùng lần cuối ${date}',
      neverUsed: 'Chưa dùng lần nào',
      expires: 'Hết hạn ${date}',
      noExpiry: 'Không hết hạn',
      error: 'Không thể tải danh sách token',
      creating: 'Đang tạo...',
      done: 'Xong',
    },
    explain: {
      button: 'Giải thích AI',
      dialogTitle: 'Giải thích bằng AI',
      loading: 'Đang tạo giải thích, có thể mất một lúc...',
      save: 'Lưu vào thư viện',
      saving: 'Đang lưu...',
      savedButton: 'Đã lưu vào thư viện',
      regenerate: 'Tạo lại',
      feedbackPlaceholder: 'Góp ý để cải thiện phiên bản tiếp theo (không bắt buộc)...',
      error: 'Không thể tạo giải thích',
      version: 'Phiên bản ${version}',
    },
  },

  chat: {
    activeNow: 'Đang hoạt động',
    sayHi: 'Hãy chào để bắt đầu cuộc trò chuyện!',
    minimize: 'Thu nhỏ',
    close: 'Đóng',
    chats: 'Chats',
    search: 'Tìm kiếm trong Messenger',
    noConversations: 'Chưa có cuộc trò chuyện nào',
    all: 'Tất cả',
    unread: 'Chưa đọc',
    newChat: 'Tin nhắn mới',
    selectConversation: 'Chọn một cuộc trò chuyện',
    selectConversationDesc:
      'Chọn từ các cuộc trò chuyện hiện có, hoặc bắt đầu cuộc trò chuyện mới.',
    startNewChat: 'Bắt đầu cuộc trò chuyện mới',
    searchFriends: 'Tìm bạn bè...',
    noFriendsToMessage: 'Không có bạn bè để nhắn tin',
    addFriendsFirst: 'Thêm bạn bè trước để bắt đầu nhắn tin.',
    creating: 'Đang mở...',
    you: 'Bạn',
  },
};
